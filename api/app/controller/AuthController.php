...
class AuthController{
        ...
        if(isset($_FILES['image'])){
           ...
           if(move_uploaded_file($tmp_name,"images/".$new_img_name)){
               ...
               $insert_query = mysqli_query($this->conn->connect(),
                   "INSERT INTO user (unique_id, fname, lname, email, password, img, status)
               ...
               $select_sql2 = mysqli_query($this->conn->connect(), "SELECT * FROM user WHERE email = '${email}'");
    ...
    public function logIn(){
        ...
        $sql = mysqli_query($this->conn->connect(), "SELECT * FROM user WHERE email = '${email}'");
        ...
        $sql2 = mysqli_query($this->conn->connect(),
            "UPDATE user SET status = '{$status}' WHERE unique_id = {$row['unique_id']}");
        if($sql2){
    ...
    public function logOut(){
        ...
        if(isset($logout_id)){
            ...
            $sql = mysqli_query($this->conn->connect(),
                "UPDATE user SET status = '{$status}' WHERE unique_id={$logout_id}");
            if($sql){
                session_unset();
                session_destroy();
                header("location: ../login.php");
